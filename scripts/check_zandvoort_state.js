...
async function main() {
  const { data: events, error: e1 } = await sb
    ...
    .order('sort_order');
  if (e1) throw e1;
  console.log('=== CALENDAR EVENTS (tutti) ===');
  events.forEach(ev => console.log(
    ` ${String(ev.sort_order).padStart(3)} | ${(ev.location || '').padEnd(16)} | ${ev.event_type || ''} | ${ev.event_date || ev.date || ''} | sprint:${ev.is_sprint ?? ''} | id:${ev.id}`
  ));

  const { data: races, error: e2 } = await sb.from('races').select('*');
  if (e2) throw e2;
  console.log('\n=== RACES ===');
  races.forEach(r => console.log(` race ${r.id} | event:${r.calendar_event_id} | sprint:${r.is_sprint} | created:${r.created_at}`));

  // Formazioni per le races più recenti
  const { data: lineups, error: e3 } = await sb.from('lineups').select('race_id, team_id, is_reserve');
  if (e3) throw e3;
  const byRace = {};
  ...
  const { data: results, error: e4 } = await sb.from('race_results').select('race_id');
  if (e4) throw e4;
  const resByRace = {};
