import { POINTS_TABLE, OVERTAKE_BONUS, MAX_OVERTAKE_POINTS, FASTEST_LAP_BONUS, DOTD_POINTS, PENALTIES } from './data';

export const SPRINT_POINTS_TABLE = { 1: 8, 2: 7, 3: 6, 4: 5, 5: 4, 6: 3, 7: 2, 8: 1 };
 ...
 * @param {Boolean} isSprint - Se vero, applica logica Sprint.
 * @returns {Object} { total, base, overtakes, fastestLap, dotd }
 */
export function calculatePilotPoints(result, isSprint = false) {
  const breakdown = { total: 0, base: 0, overtakes: 0, fastestLap: 0, dotd: 0 };

  // Driver of the Day è una mini-sfida accessoria, NON un punto-gara:
  // vale anche se il pilota è DNF. (Non previsto nelle Sprint.)
  if (!isSprint && result.dotdRank && DOTD_POINTS[result.dotdRank]) {
    breakdown.dotd = DOTD_POINTS[result.dotdRank];
    breakdown.total += breakdown.dotd;
  }

  // DNF: niente punti piazzamento/sorpassi, resta solo l'eventuale DOTD sopra.
  if (result.dnf) return breakdown;

  if (isSprint) {
    breakdown.base = SPRINT_POINTS_TABLE[result.position] || 0;
    breakdown.total += breakdown.base;
    return breakdown; // Nessun bonus previsto nelle Sprint
  }
...
export function calculateTeamScores(teams, pilots, races, lineups, reserves = {}) {
  ...
  teams.forEach(t => {
    ...
    // Penalità manuali (deduzioni stagionali, es. mancato schieramento in tempo)
...
export function calculateRaceTeamScore(race, lineups, reserves, pilots, teamId) {
  ...
  // Process starters: la formazione è quella originalmente schierata.
  // DNF di un titolare → 0 pt (NESSUNA sostituzione automatica della riserva).
  // L'unico modo per far entrare la riserva è uno switch MANUALE post-gara.
  teamLineups.forEach(starterEntry => {
    const result = race.results.find(r => r.pilotId === starterEntry.id);
    if (!result) return;

    // Se questo titolare è stato manualmente sostituito (switch admin),
    // i suoi punti non contano: contano quelli della riserva.
    if (teamReserveObj && starterEntry.subbedOutFor === teamReserveObj.id) {
      return;
    }

    // DNF → 0 pt-gara, MA l'eventuale DOTD viene comunque conteggiato
    // (gestito dentro calculatePilotPoints, che sui DNF ritorna solo il DOTD).
    total += calculatePilotPoints(result, race.isSprint).total;
  });

  // Riserva: entra in classifica solo se manualmente subbed in.
  if (teamReserveObj && teamReserveObj.subbedInManually) {
    const resResult = race.results.find(r => r.pilotId === teamReserveObj.id);
    if (resResult) {
      total += calculatePilotPoints(resResult, race.isSprint).total;
    }
  }
...
export function getRaceBreakdown(race, pilots) {
  ...
  // Include sia i classificati che i DNF, cosi la vista piloti mostra tutti i 22.
  return race.results
    .slice()
    .sort((a, b) => {
    ...
    .map(result => {
      ...
      const breakdownObj = calculatePilotPoints(result, race.isSprint);
