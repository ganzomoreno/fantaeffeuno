...
export async function openNextAuction(budgetToAdd) {
  ...
  // 5. Re-draft: libera automaticamente TUTTI i piloti per la nuova asta.
  //    Ogni asta è un draft completo: il budget di partenza è già
  //    (residuo asta precedente + budgetToAdd), quindi il costo rosa NON si
  //    rimborsa. Da qui l'admin riassegna i piloti dalla regia (GestioneAste).
  const { error: eFree } = await supabase
    .from('pilots')
