...
export const selectStyle = {
  ...
  color: "#fff", padding: "8px 12px", fontSize: 17, width: "100%",
};

export const inputStyle = {
  background: "#1a1a1a", border: "1px solid #333", borderRadius: 8,
  color: "#fff", padding: "8px 12px", fontSize: 17,
};

export const btnPrimary = {
  background: "#e10600", border: "none", borderRadius: 8,
  color: "#fff", padding: "10px 20px", cursor: "pointer",
  fontWeight: 700, fontSize: 17, textTransform: "uppercase", letterSpacing: 1,
};

export const btnSmall = {
  background: "transparent", border: "1px solid #333", borderRadius: 6,
  color: "#aaa", padding: "4px 10px", cursor: "pointer", fontSize: 15, fontWeight: 600,
};
...
export function Chip({ label, color }) {
  return (
    <span style={{
      fontSize: 14, fontWeight: 700, padding: "3px 10px",
...
export function MiniInput({ label, type = "text", value, onChange, width = 100 }) {
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      {label && <span style={{ fontSize: 14, opacity: 0.5, marginBottom: 2 }}>{label}</span>}
      <input
        type={type}
        value={value}
        onChange={e => onChange(e.target.value)}
        style={{ ...inputStyle, width, padding: "4px 8px", fontSize: 16 }}
      />
    </div>
  );
}
